{{ config(
    materialized='table',
    tags=['marts', 'fraud'],
    description='Daily summary of transaction and fraud metrics; one row per transaction_date.'
) }}
...
daily AS (
    SELECT
        -- Grain: all transactions that occurred on this calendar date
        transaction_date,
        ...
        -- Daily fraud counts and fraud amount
        ...
        -- Daily count of 3-sigma outliers and transactions in the highest risk band
        ...
        -- Daily counts of priority transactions by reason
        ...
        -- Daily total of all priority transactions (combined flag)
        SUM(CASE WHEN is_priority_review THEN 1 ELSE 0 END) AS priority_review_transactions_count,

        -- Daily count of transactions that occurred in off-peak hours
...
final AS (
    SELECT
        ...
        -- Core daily volume metrics
        transactions_count,
        ROUND(total_amount, 2) AS total_amount,
        ROUND(avg_transaction_amount, 2) AS avg_transaction_amount,

        -- Daily fraud counts, fraud amount, and fraud rate
        ...
        -- Daily anomaly and priority metrics
        ...
        -- Daily count of off-peak transactions
        off_peak_transactions_count

    FROM daily
)

-- Final mart: one row per transaction_date with daily fraud and anomaly KPIs
SELECT *
FROM final
